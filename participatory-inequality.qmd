...
Our data cover over eight hundred regulatory actions (part of `r n_rules` rulemaking processes) across seven agencies.<!--^[We define a regulatory action as publishing a proposed or final rule by one agency. We count one joint rule issued by the SEC and the Federal Reserve as two regulatory actions.] -->
<!--2. METHODS-->
...
<!---FINDINGS 
We show that wealthier organizations enjoy material advantages in administrative policymaking. 
---> 
We show that wealthy organizations are more likely to participate in administrative policymaking, and even when the less wealthy participate, wealthy organizations are more likely to have their demands met.
...
Research also shows that business interests are influential in rulemaking. <!--@Libgober2020 shows firms that engaged regulators via meeting and commenting achieved abnormally positive stock returns upon the announcement of rules on which they lobbied, that the positive returns associated with lobbying were still worth many billions of dollars even just focusing on comparisons within narrow sets of competitive firms, and further that the extent to which regulators gave commenters what they asked for also predicted the initial stock market reaction to rule announcement. While these findings make a case for firm influence over rulemaking, they only do so within a very elite subpopulation of influence seekers: publicly traded firms. Other studies of influence over rulemaking have focused more on the differences between business commenters and those representing other, often more general, interests. For example,--> Comments from businesses on proposed transportation and labor regulations better predicted policy changes than non-business comments [@YackeeJOP2006]. Similarly, regulatory policy is more likely to change during U.S. Office of Management and Budget's (OMB) review when more business interests lobby OMB [@Haeder2015]. 
However, the extent to which this bias toward business interests is a result of inequalities in the resources that businesses and non-business interests have for lobbying agencies is less clear.
...
# out-extra: "\\floatnote{Counts of regulatory acts by agency and by year. Regulatory acts include ANPRM, NPRM, Interim Rule, and Final Rules. Only actions implementing Dodd-Frank are included. Note that one complete rulemaking process typically has two and sometimes more associated actions.}"
...
#| out.width: 80%
# out-extra: "\\floatnote{Counts of distinct organizations that have submitted comments to each financial regulator by organizational type. These counts reflect only those comments that have been matched to an organization. The lowest match rate (at the CFPB) still exceeds 20%. See the appendix.}"
...
#| fig-cap: "Number of Comments by Organization Type and Agency"
#| out-width: 80%
# out-extra: "\\floatnote{Counts of comments from organizations that have submitted comments to each financial regulator by organizational type. These counts reflect only those comments that have been matched to an organization. Details about the match rate are in the appendix.}"
