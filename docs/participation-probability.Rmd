...
mpNonprofit_ind_assoc <- glm(commented ~ Assets + revenue + ind_assoc,
           data = nonprofit_resources %>% mutate(ind_assoc = SUBSECTION == "06"), 
             family=binomial(link="logit"))

mpNonprofit_subsection <- glm(commented ~ Assets + revenue + SUBSECTION,
           data = nonprofit_resources, 
             family=binomial(link="logit"))
...
```{r,  mp-all-predict-faceted, fig.height=4.5, fig.width=3.5}
...
predicted <- marginaleffects::predictions(mpAlliquad,
                         newdata = values) %>% 
  filter(org_type != "Industry assoc.")
