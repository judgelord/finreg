---
title: "Efficacy in OCC Rulemaking"
...
technical_terms_comments %<>% 
  distinct()  %>%
  filter(str_detect(comment_url, "OCC-"))
...
# check for OCC comments (missing as of June 2024)
efficacy_raw %>% filter(str_detect(comment_url, "OCC")) |> head()

efficacy_raw %>% filter(str_detect(attachment_url, "OCC")) |> head()

efficacy_raw |> drop_na(url) |>  mutate( agency = str_extract(url , ".*gov..........")) %>% pull(agency) %>% unique()

# check for OCC comments (present as of May 2023)
attachments %>% filter(str_detect(comment_url, "OCC")) |> head()
attachments %>% filter(str_detect(attachment_url, "OCC")) |> head()
...
save(models, rows,
     file = here::here("models", "OCC", "mes.Rdata"))
...
save(models, rows,
     file = here::here("models", "OCC", "mes-pois.Rdata"))
```
---
...
save(models.med1, rows, file = here::here("models", "OCC", "models.med2.Rdata"))
...
save(models.med.spending, rows, file = here::here("models", "OCC", "models.med.spending.Rdata"))
...
save(models.m4, rows, file = here::here("models", "OCC", "models.m4.Rdata"))
...
save(models.m4No0, rows, file = here::here("models", "OCC", "models.m4No0.Rdata"))
...
save(models.m, rows, file = here::here("models", "OCC", "models.m.Rdata"))
```
