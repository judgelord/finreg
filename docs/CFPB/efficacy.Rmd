---
title: "Efficacy in CFPB Rulemaking"
...
technical_terms_comments %<>% 
  distinct()  %>%
  filter(str_detect(comment_url, "CFPB-"))
...
# example
bluebook_attachments %>% filter(comment_url == "https://www.regulations.gov/comment/CFPB-2011-0008-0126") %>% kablebox()

bluebook_comments %>% filter(comment_url == "https://www.regulations.gov/comment/CFPB-2011-0008-0126") %>% kablebox()
...
technical_terms %<>% filter(!comment_url %in% c(
  "https://www.regulations.gov/comment/CFPB-2013-0008-0028"
))
...
boxplotXefficacyXsophistication <- efficacyXsophistication %>% 
  ...
  labs(caption = str_c("Comments on Dodd-Frank Rules, N = ", nrow(efficacyXsophistication) %>% prettyNum(big.mark = ",")),
       y = "Lobbying Success\n(Words From Comment\nAdded to Final Rule)",
       x = "Technical Terms in Comment (CFPB Comments Only)",
...
p <- b %>% 
  ...
  labs(x = "Mean PAC Contribution (Thousands USD)",
       y = "Legal Sophistication",
       title = "(CFPB Only) Campaign Donors")
...
p <- b %>% 
  ...
  labs(x = "Market Capitalization (Billions)",
       y = "Legal Sophistication",
       title = "(CFPB Only) Publicly-Traded Companies")
...
IA_legal <- b %>% 
  ...
  labs(x = "Assets (Millions USD)",
       y = "Legal Citations", 
       title = "(CFPB Only) Industry Associations")
...
save(models, rows,
     file = here::here("models", "CFPB", "mes.Rdata"))
...
save(models, rows,
     file = here::here("models", "CFPB", "mes-pois.Rdata"))
```
...
save(models.med1, rows, file = here::here("models", "CFPB", "models.med2.Rdata"))
...
save(models.med.spending, rows, file = here::here("models", "CFPB", "models.med.spending.Rdata"))
...
save(models.m4, rows, file = here::here("models", "CFPB", "models.m4.Rdata"))
...
save(models.m4No0, rows, file = here::here("models", "CFPB", "models.m4No0.Rdata"))
...
acme %>% 
  ...
  geom_pointrange(position = position_dodge(width = 0.30),
                  alpha = .8) + 
  labs(title = "(CFPB Only) Mediation",
...
save(models.m, rows, file = here::here("models", "CFPB", "models.m.Rdata"))
```
...
- [ ] check on the status of CFPB comments (missing from the attachments table as of May 2022)
