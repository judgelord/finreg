...
nonprofit_resources %>% 
  ...
  scale_x_log10() + 
  labs(title = "Nonprofits",fill = "", x = "Assets (Millions)") + 
...
creditunions %>% 
  ...
  geom_vline( aes(xintercept = mean_assets/1000000)) + 
  scale_x_log10() + 
  labs(title = "Credit Unions",fill = "", x = "Assets (Millions)") + 
  facet_grid(Commented ~ ., scales = "free_y")+
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
```
...
```{r mp-nonprofits}
nonprofit_resources %<>% mutate(Assets = assets/1000000)
...
```{r mp-credit-unions}
creditunions %<>% mutate(Assets = assets/1000000)
