...
The root for this script is `finreg`. Most of the data are in `finreg/data`, but some are up one level in `FINREGRULEMAKE2/Data`. This can and should be streamlined significantly. 
...
- Market cap: from compustat `data/merged_resources/compustat_resources.csv` and SEC's CIK database `Data/CIK.csv` 
...
nonprofit_resources %>% distinct(Commented, mean_assets, mean_revenue)
...
nonprofit_resources %>% 
  ...
  # geom_text(x = aes(mean_revenue/1000), 
  #            y =100) + # nonprofit_resources %>% filter(revenue > 10) %>% nrow() ) + 
  scale_x_log10() + 
...
```{r, fig.height= 1.7, fig.width=4, out.width="50%"}
...
creditunions %>% 
  ...
  geom_density(alpha = .5, color = NA) + 
  scale_x_log10() + 
  labs(fill = "", x = "Assets", title = "Credit Unions")
...
creditunions %>% distinct(Commented, mean_assets)
...
creditunions %>% 
  ...
  geom_vline( aes(xintercept = mean_assets/1000)) + 
  scale_x_log10() + 
  labs(title = "Credit Unions",fill = "", x = "Assets ($1,000s)") + 
  facet_grid(Commented ~ ., scales = "free_y")+
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank())
```
...
d %<>% mutate(
  assets = coalesce(`nonprofits_resources-bestMatch:assets`, assets),
  revenue = coalesce(`nonprofits_resources-bestMatch:revenue`, revenue)
)
...
```{r}
d %>% filter(str_detect(best_match_name, "petroleum inst|yale uni")) %>%
  select(comment_url, best_match_name, org_resources, #opensecrets_resources_name,
...
TODO 
```{r mp-credit-unions, eval=FALSE}
creditunions %<>% mutate(Assets = assets)
