...
I changed a few names from Jacob's asset data: 
```{r}
...
d %<>% mutate(Best_match_type = best_match_type %>% 
                str_replace("CIK", "Filed with SEC (CIK Data)") %>% 
                str_replace("compustat", "Wharton Compustat Company Database") %>% 
                str_replace("FDIC", "FDIC-Insured Bank") %>% 
                str_replace("Nonprofit", "Nonprofit (IRS Data)") %>% 
...
d %>% 
  ...
  select(org_comments, everything()) %>% 
  kablebox()
```
...
We only have market cap for 10 firms?
```{r}
# fishy
d %>% 
  ggplot() +
...
mp <- glm(commented ~ org_resources,
           data = d %>% mutate(org_resources = org_resources/10000), 
             family=binomial(link="logit"))
...
mb_assets <- lm(Total_Legal_Citations ~ assets,
           data = b  %>% mutate(assets = assets/10000))
...
mb_compustat <- glm(Total_Legal_Citations ~ marketcap2,
           data = b  %>% mutate(marketcap2 = marketcap2/10000))

models <- list(mb_assets, mb_opensecrets, mb_compustat)
...
mt_assets <- lm(dictionary_terms ~ assets,
           data = t  %>% mutate(assets = assets/10000))
...
mt_compustat <- glm(dictionary_terms ~ marketcap2,
           data = t  %>% mutate(marketcap2 = marketcap2/10000))

models <- list(mt_assets, mt_opensecrets, mt_compustat)
