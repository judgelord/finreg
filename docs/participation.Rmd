...
## Among organizations that commented on a Dodd-Frank rule
...
asset_counts %>% 
  ...
  geom_bar() + 
  labs(title = "Banks and Nonprofits",
       y = "Number of Organizations",
       fill = "Frequency of\nCommenting",
...
asset_counts %>% 
  ...
  labs(y = "Number of Companies",
       title = "Publicaly-Traded Companies",
       fill = "Frequency of\nCommenting",
       x = "Number of Dodd-Frank Rules") #+ scale_y_log10()
...
asset_counts %>% 
  ...
  geom_density(alpha = .5, color = NA) + 
  scale_x_log10() + 
  labs(title = "Publicaly-Traded Companies",
...
models <- list(#`All` = m_asset_count, 
               `Banks` = m_fdic_count,
               `Nonprofits` = m_nonprofit_count,
               `Firms` = m_compustat_count,
...
t.test(FDIC_resources %>% filter(commented) %>% pull(ASSET),
       FDIC_resources %>% filter(!commented) %>% pull(ASSET))


t.test(FDIC_resources %>% filter(commented, BKCLASS == "NM") %>% pull(ASSET),
       FDIC_resources %>% filter(!commented, BKCLASS == "NM") %>% pull(ASSET))


t.test(FDIC_resources %>% filter(commented, BKCLASS == "SM") %>% pull(ASSET),
       FDIC_resources %>% filter(!commented, BKCLASS == "SM") %>% pull(ASSET))


t.test(FDIC_resources %>% filter(commented, BKCLASS == "SB") %>% pull(ASSET),
       FDIC_resources %>% filter(!commented, BKCLASS == "SB") %>% pull(ASSET))


t.test(FDIC_resources %>% filter(commented, BKCLASS == "SA") %>% pull(ASSET),
...
t.test(nonprofit_resources %>% filter(commented) %>% pull(assets),
...
t.test(creditunions %>% filter(!commented) %>% pull(assets),
...
t.test(compustat %>% filter(commented) %>% pull(marketcap2),
