---
title: "Participation in Financial Rulemaking" #subtitle: "Appendix and Replication Code" 
output:
...
```{r asset-counts, fig.width=4, out.width= "50%"}
asset_counts <- d %>% 
...
```{r nonprofit-rules, fig.width=4, out.width= "50%"}
asset_counts <- d %>% 
...
```{r fdic-rules, fig.width=4, out.width= "50%"}
asset_counts <- d %>% 
...
```{r compustat-rules, fig.width=4, out.width= "50%"}
# compustat-rules
...
asset_counts %>% 
  ggplot() + 
  aes(x = marketcap, fill = onepercent) + 
  geom_density(alpha = .5, color = NA) + 
  scale_x_log10() + 
...
```{r opensecrets-rules, fig.width=4, out.width= "50%"}
asset_counts <- d %>% 
...
asset_counts %>% 
  ...
  scale_x_log10() + 
  labs(title = "Campaign Donors",
       fill = "", y = "", x = "PAC Donations (Thousands)")+
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
        axis.text.y = element_blank())
