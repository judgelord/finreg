...
opensecrets %<>% 
  mutate(org_name = orgName %>% str_to_lower(),
         Commented = ifelse(
    org_name %in% d$best_match_name | org_name %in% d$organization | org_name %in% d$opensecrets_resources_name | parentID %in% d$parentID, # FIXME this name does not exactly match across datasets 
    "Commented", 
    "Did not comment"))
...
compustat %<>% mutate(
  ...
  commented = cik %in% d_companies$cik | org_name %in% d_companies$organization,
  Commented = ifelse(commented, "Commented", "Did not comment")
)
...
values <- all %>% 
  #filter(org_type == "Bank") %>% 
  tidyr::expand(assets_b = seq(0,1,.1), org_type) %>% 
