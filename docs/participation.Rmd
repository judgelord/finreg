...
- Nonprofit assets and revenue: `finreg/data/nonprofit_resources.Rdata`
- FDIC `finreg/data/FDIC_resources.Rdata`
- Credit unions: `Data/creditunions.csv` (up one level, not in finreg)
- Campaign donations: `finreg/data/merged_resources/opensecrets_resources_JwVersion.csv`
- Market cap: from compustat `finreg/data/merged_resources/compustat_resources.csv` and SEC's CIK database `Data/CIK.csv` 
...
Counts
```{r FDIC-count, fig.height=2.5, fig.width=4, out.width="50%"}
# FDIC-count
FDIC_resources %<>% 
...
FDIC_resources %>% distinct(Commented, BKCLASS, mean_ASSET_type) %>% arrange(BKCLASS) %>% kable()


FDIC_resources %>%
  filter(ASSET > 10) %>% 
  mutate(Commented = Commented %>% paste("\n(mean assets =", round(mean_ASSET, 0) %>% prettyNum(big.mark = ","), ")")) %>% 
  ggplot() + 
...
FDIC_resources %>% filter(BKCLASS == "N") %>% distinct(org_name)
```

Omitting class N and IO (Class IO are foreign banks and did not comment)

N means "national association"

```{r FDIC-count-select, fig.height=1.5, fig.width=4, out.width="50%"}
FDIC_resources %<>% 
  filter(BKCLASS != "N", BKCLASS != "OI") %>%
  group_by(Commented) %>% 
  mutate(mean_ASSET = mean(ASSET, na.rm =TRUE)) %>% 
  ungroup() %>% 
  group_by(Commented, BKCLASS) %>% 
...
FDIC_resources %>%
  ...
  geom_vline( aes(xintercept = mean_ASSET/1000)) + 
  scale_x_log10() + 
  labs(title = "FDIC-Insured Banks",fill = "", y = "", x = "Assets (Thousands)") + 
  facet_wrap("Commented", ncol = 1, scales = "free_y", strip.position = "top")+
  theme(panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank(),
        legend.position = "none")
...
```{r FDIC-count-by-class, fig.height=2.5,  fig.width=4}
FDIC_resources %>%
  filter(ASSET > 10, BKCLASS %in% c("NM")) %>% 
  mutate(Commented = Commented %>% paste0("\n(mean assets = $", round(mean_ASSET_type, 0) %>% prettyNum(big.mark = ","), ")" )) %>% 
  ggplot() + 
  aes(x = ASSET/1000, fill = Commented, color = Commented) + 
  geom_histogram(alpha = .5, color = NA) + 
...
nonprofit_resources %<>% 
  ...
  mutate(mean_assets = mean(assets, na.rm = T),
         mean_revenue = mean (revenue, na.rm = T)) %>% 
  ungroup()
...
# mean assets
mean_commented <- nonprofit_resources %>% filter(Commented == "Commented") %>% distinct(mean_assets/1000000) %>% round(0) %>%  paste("million")

mean_not_commented <- nonprofit_resources %>% filter(Commented != "Commented") %>% distinct(mean_assets/1000000) %>% round(0) %>%  paste("million")
```
...
```{r nonprofit-count, fig.height=2.5, fig.width=4, out.width="50%"}
#nonprofit-count  histograms 
nonprofit_resources %>% 
  filter(assets > 10) %>% 
  mutate(Commented = Commented %>% paste("\n(mean assets =", round(mean_assets/10000000, 0), "million)")) %>% 
  ggplot() + 
...
creditunions %>% 
  ...
  geom_density(alpha = .5, color = NA) + 
  scale_x_log10() + 
  labs(fill = "", x = "Assets (Millions)", title = "Credit Unions")
```


```{r creditunion-count, fig.height= 2.5, fig.width=4, out.width="50%"}
# creditunion-count 
creditunions %<>% 
  group_by(Commented) %>% 
  mutate(mean_assets = mean(assets, na.rm = T) ) %>% 
...
These models have the prefix `memt-`. The dependent variable is the efficacy score. The mediator is technical terms.
...
# Create two data frames, one wide and one long ####
