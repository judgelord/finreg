...
opensecrets %<>% mutate(MeanContribAmount_m = MeanContribAmount/1000000,
                        TotalContribAmount_m = TotalContribAmount/1000000)

mpOpensecrets <- glm(commented ~ MeanContribAmount_m,
           data = opensecrets, 
             family=binomial(link="logit"))

mpOpensecrets_total <- glm(commented ~ TotalContribAmount_m,
           data = opensecrets, 
             family=binomial(link="logit"))
...
values <- opensecrets %>% 
  ...
  tidyr::expand(MeanContribAmount = seq(0,100000,1000) )#, org_type = "Bank") 
...
p <- predicted %>% 
  ggplot() + 
  aes(x = MeanContribAmount, y = estimate#, shape = org_type, color = org_type
      ) + 
