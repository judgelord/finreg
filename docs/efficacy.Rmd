...
File: `Data/Dictionary_Terms.RData` contains 6 objects (3 sets of measures for both comments and attachments)
...
d %>% arrange(marketcap2) %>% 
  dplyr::select(org_name, marketcap2, comment_url) %>% kablebox()
...
d %>% 
  dplyr::select(comment_url, org_name, Total_Legal_Citations, dictionary_terms) %>% 
...
modelplot( mea_assets) + 
  labs(x = "Additional Words Added to Final Rule per Million Dollars")

modelplot( mea_opensecrets) + 
  labs(x = "Additional Words Added to Final Rule per Thousand Dollars")

modelplot(mea_marketcap) + 
  labs(x = "Additional Words Added to Final Rule per Million Dollars")
```
...
modelplot(mes_terms) + 
  labs(x = "Additional Words Added to Final Rule per Technical Term")

modelplot( mes_bluebook ) + 
  labs(x = "Additional Words Added to Final Rule per Legal Citation")

modelplot(mes_terms_bluebook ) + 
  labs(x = "Additional Words Added to Final Rule")
...
To assess sophistication as a mediator between wealth and the influence of public comments on rules (efficacy), we estimate the average conditional marginal effect (ACME, conditional on the the number of technical terms or legal citations) and average direct effect (ADE) of wealth using mediation analysis. 
...
```{r mediation-marketcap-bluebook}
# model predicting mediator, sophistication 
...
- [ ] check on status of OCC comments (missing from attachments table as of May 2022)

- [ ] split out FDIC banks and nonprofits in asset mediation analysis? (at the moment, neither type of assets are correlated with efficacy, so mediation analysis is a low priority)

- [ ] Try to get efficacy measures for key obaservations. For example, cases where we have market cap: 
