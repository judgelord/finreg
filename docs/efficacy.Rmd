...
efficacy_raw %>% count(is.na(url))
# check for OCC comments (missing as of May 2022)
# 
efficacy_raw %>% filter(str_detect(comment_url, "OCC"))
attachments %>% filter(str_detect(comment_url, "OCC"))


# merge in comment url from attachments table 

# drop empty url
efficacy_raw %<>% select(-comment_url)
...
#FIXME 
efficacy_raw %<>% left_join(attachments)
...
# plot(med.cont.pac, main = "PAC Spending",   xlab = "Words From Comment Added to Final Rule\nPer Million Dollars in Assets")
...
plot(med.cont.pac, main = "PAC Spending", 
     xlab = "Words From Comment Added to Final Rule\nPer Million Dollars in Assets")
...
med.cont <- mediate(model.m, model.y, sims=1000, treat = "marketcap_b",
mediator = "dictionary_terms")
...
```{r mediation-4wayNo0-acme, fig.width=4.5, fig.height=1.65}
#mediation-4way-acme
