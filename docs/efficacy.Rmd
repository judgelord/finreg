...
```{r efficacyXsophistication boxplot with label}
boxplotXefficacyXsophistication <- efficacyXsophistication %>% 
  ...
  labs(caption = str_c("Comments on Dodd-Frank Rules, N = ", nrow(efficacyXsophistication) %>% prettyNum(big.mark = ",")),
       y = "Lobbying Success\n(Words From Comment\nAdded to Final Rule)",
       x = "Binned Technical Terms in Comment",
       color = "") +
...
NP_tech <- t %>% 
  ...
  labs(x = "Binned Assets (Millions USD)",
       y = "Technical Terms", 
       title = "Nonprofits")
...
CU_tech <- t %>% 
  filter(!is.na(CreditUnions_name) & !is.na(assets) & assets > 0) %>%
  mutate(
    log_dictionary_terms = log10(dictionary_terms + 1),
    assets_in_millions = assets/1000000,
    log_assets_in_millions = log10(assets/1000000),
...
PTC_tech <- t %>% 
  ...
  labs(x = "Binned Market Capitalization (Millions USD)",
       y = "Technical Terms", 
       title = "Publicly-traded Companies")
...
ggsave(plot = NP + theme(axis.title = element_text(size = 14)), filename = "figs/boxplot-assets-efficacy-2.png",bg="white")
ggsave(plot = CU + theme(axis.title = element_text(size = 14)), filename = "figs/boxplot-assets-efficacy-4.png",bg="white")
ggsave(plot = PTC + theme(axis.title = element_text(size = 14)), filename = "figs/boxplot-assets-efficacy-6.png",bg="white")
ggsave(plot = FDIC_Banks + theme(axis.title = element_text(size = 14)), filename = "figs/boxplot-assets-efficacy-8.png",bg="white")
...
```{r mediation-4wayNo0-acme, fig.width=6.5, fig.height=1.75}
#mediation-4way-acme
