---
title: "Efficacy in CFTC Rulemaking"
...
technical_terms_comments %<>%
  filter(str_detect(comment_url, "cftc"))
...
#efficacy %>% filter(str_detect(comment_url, "CFTC")) %>%  kablebox()
...
efficacy %>% filter(str_dct(comment_url,"CFTC-2016-0020-0008"))
...
# example
bluebook_attachments %>% filter(comment_url == "https://www.regulations.gov/comment/CFTC-2011-0008-0126") %>% kablebox()

bluebook_comments %>% filter(comment_url == "https://www.regulations.gov/comment/CFTC-2011-0008-0126") %>% kablebox()
...
# bluebook %>% filter(str_detect(comment_url, "CFTC-2019-0021-0406"))
# technical_terms %>% filter(str_detect(comment_url, "CFTC-2019-0021-0406"))
...
efficacy%<>% filter(!comment_url %in% c(
  ...
  "https://www.regulations.gov/comment/CFTC-2012-0029-0054"
))
...
technical_terms %<>% filter(!comment_url %in% c(
  "https://www.regulations.gov/comment/CFTC-2013-0008-0028"
))
...
boxplotXefficacyXsophistication <- efficacyXsophistication %>% 
  ...
  labs(caption = str_c("Comments on Dodd-Frank Rules, N = ", nrow(efficacyXsophistication) %>% prettyNum(big.mark = ",")),
       y = "Lobbying Success\n(Words From Comment\nAdded to Final Rule)",
       x = "Technical Terms in Comment (CFTC Comments Only)",
...
p <- b %>% 
  ...
  labs(x = "Mean PAC Contribution (Thousands USD)",
       y = "Legal Sophistication",
       title = "(CFTC Only) Campaign Donors")
...
p <- b %>% 
  ...
  labs(x = "Market Capitalization (Billions)",
       y = "Legal Sophistication",
       title = "(CFTC Only) Publicly-Traded Companies")
...
IA_legal <- b %>% 
  ...
  labs(x = "Assets (Millions USD)",
       y = "Legal Citations", 
       title = "(CFTC Only) Industry Associations")
...
save(models, rows,
     file = here::here("models", "CFTC", "mes.Rdata"))
...
save(models, rows,
     file = here::here("models", "CFTC", "mes-pois.Rdata"))
```
...
save(models.med1, rows, file = here::here("models", "CFTC", "models.med2.Rdata"))
...
save(models.med.spending, rows, file = here::here("models", "CFTC", "models.med.spending.Rdata"))
...
save(models.m4, rows, file = here::here("models", "CFTC", "models.m4.Rdata"))
...
save(models.m4No0, rows, file = here::here("models", "CFTC", "models.m4No0.Rdata"))
...
acme %>% 
  ...
  geom_pointrange(position = position_dodge(width = 0.30),
                  alpha = .8) + 
  labs(title = "(CFTC Only) Mediation",
...
save(models.m, rows, file = here::here("models", "CFTC", "models.m.Rdata"))
```
...
- [ ] check on the status of CFTC comments (missing from the attachments table as of May 2022)
