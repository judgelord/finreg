...
d <- match_data_clean  |> filter(comment_agency == "FRS")
...
d %>% distinct(Agency, org_type2, org_name)  %>% filter(Agency == "FDIC", is.na(org_type2))
 ...
 # caption = "Consumer Financial Protection Bureau (FRS)
...
d %<>% mutate(payday = ifelse(str_detect(comment_url, "FRS-2016-0025|FRS-2019-0006"), "Payday Loan Rules", "All Other Rules"))
...
ind_assoc_resources %>% 
  ...
  densityplot(var = "assets",
              by = 1000,
              title = "Industry Associations (FRS Only)",
               x = "Assets (Thousands)",
...
opensecrets %>% 
  densityplot(var = "MeanContribAmount",
              by = 1000,
              title = "Campaign Donors (FRS Only)",
...
opensecrets %>% 
  densityplot(var = "MeanLobbyingAmount",
              by = 1000,
              title = "Lobbying Spending (FRS Only)",
...
compustat %>% 
  ...
  densityplot(var = "marketcap2",
              by = 1000000, 
              title = "Publicly-traded Companies (FRS Only)",
...
Continued at judgelord.github.io/finreg/FRS/participation-probability.html and judgelord.github.io/finreg/FRS/participation-frequency.html
