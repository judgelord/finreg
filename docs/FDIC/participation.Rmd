...
d <- match_data_clean  |> filter(comment_agency == "FDIC")
...
d %>% distinct(Agency, org_type2, org_name)  %>% filter(Agency == "FDIC", is.na(org_type2))
 ...
 # caption = "Consumer Financial Protection Bureau (FDIC)
...
d %<>% mutate(payday = ifelse(str_detect(comment_url, "FDIC-2016-0025|FDIC-2019-0006"), "Payday Loan Rules", "All Other Rules"))
...
ind_assoc_resources %>% 
  ...
  densityplot(var = "assets",
              by = 1000,
              title = "Industry Associations (FDIC Only)",
               x = "Assets (Thousands)",
...
opensecrets %>% 
  densityplot(var = "MeanContribAmount",
              by = 1000,
              title = "Campaign Donors (FDIC Only)",
...
opensecrets %>% 
  densityplot(var = "MeanLobbyingAmount",
              by = 1000,
              title = "Lobbying Spending (FDIC Only)",
...
compustat %>% 
  ...
  densityplot(var = "marketcap2",
              by = 1000000, 
              title = "Publicly-traded Companies (FDIC Only)",
...
Continued at judgelord.github.io/finreg/FDIC/participation-probability.html and judgelord.github.io/finreg/FDIC/participation-frequency.html
