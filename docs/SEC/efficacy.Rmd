---
title: "Efficacy in SEC Rulemaking"
...
technical_terms_comments %<>% 
  distinct()  %>%
  filter(str_detect(comment_url, "sec.gov"))

# source
technical_terms_comments$comment_url |> str_extract(".*gov") |> unique()
...
# source
technical_terms_attachments$comment_url |> str_extract(".*gov") |> unique()
...
# source
bluebook_attachments$comment_url |> str_extract(".*gov") |> unique()
...
# source
bluebook_comments$comment_url |> str_extract(".*gov") |> unique()
```
...
#efficacy %>% filter(str_detect(comment_url, "SEC")) %>%  kablebox()
...
efficacy %>% filter(str_dct(comment_url,"SEC-2016-0020-0008"))
...
# example
bluebook_attachments %>% filter(comment_url == "https://www.regulations.gov/comment/SEC-2011-0008-0126") %>% kablebox()

bluebook_comments %>% filter(comment_url == "https://www.regulations.gov/comment/SEC-2011-0008-0126") %>% kablebox()
...
# bluebook %>% filter(str_detect(comment_url, "SEC-2019-0021-0406"))
# technical_terms %>% filter(str_detect(comment_url, "SEC-2019-0021-0406"))
...
efficacy%<>% filter(!comment_url %in% c(
  ...
  "https://www.regulations.gov/comment/SEC-2012-0029-0054"
))
...
technical_terms %<>% filter(!comment_url %in% c(
  "https://www.regulations.gov/comment/SEC-2013-0008-0028"
))
...
boxplotXefficacyXsophistication <- efficacyXsophistication %>% 
  ...
  labs(caption = str_c("Comments on Dodd-Frank Rules, N = ", nrow(efficacyXsophistication) %>% prettyNum(big.mark = ",")),
       y = "Lobbying Success\n(Words From Comment\nAdded to Final Rule)",
       x = "Technical Terms in Comment (SEC Comments Only)",
...
p <- b %>% 
  ...
  labs(x = "Mean PAC Contribution (Thousands USD)",
       y = "Legal Sophistication",
       title = "(SEC Only) Campaign Donors")
...
p <- b %>% 
  ...
  labs(x = "Market Capitalization (Billions)",
       y = "Legal Sophistication",
       title = "(SEC Only) Publicly-Traded Companies")
...
IA_legal <- b %>% 
  ...
  labs(x = "Assets (Millions USD)",
       y = "Legal Citations", 
       title = "(SEC Only) Industry Associations")
...
save(models, rows,
     file = here::here("models", "SEC", "mes.Rdata"))
...
save(models, rows,
     file = here::here("models", "SEC", "mes-pois.Rdata"))
```
...
```{r mediation-assets-terms, fig.width=4.5, fig.height=3, eval=FALSE}
mediation_data <- b %>% 
...
```{r mediation-marketcap-bluebook, fig.width=4.5, fig.height=3, eval=FALSE}
...
```{r mediation-marketcap-terms, fig.width=4.5, fig.height=3, eval=FALSE}
...
```{r, eval=FALSE, mediation-marketcap-terms-bluebook, fig.width=4.5, fig.height=3}
# mediation-marketcap-terms-bluebook
...
save(models.med1, rows, file = here::here("models", "SEC", "models.med2.Rdata"))
...
```{r, eval = FALSE mediation-marketcap-lobbying, fig.width=4.5, fig.height=3}
...
save(models.med.spending, rows, file = here::here("models", "SEC", "models.med.spending.Rdata"))
...
```{r, eval=FALSE, mediation-marketcap-terms-lobbying, fig.width=4.5, fig.height=3}
...
save(models.m4, rows, file = here::here("models", "SEC", "models.m4.Rdata"))
...
save(models.m4No0, rows, file = here::here("models", "SEC", "models.m4No0.Rdata"))
...
save(models.m, rows, file = here::here("models", "SEC", "models.m.Rdata"))
```
...
- [ ] check on the status of SEC comments (missing from the attachments table as of May 2022)
