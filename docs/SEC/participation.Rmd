...
d <- match_data_clean  |> filter(comment_agency == "SEC")
...
d %>% distinct(Agency, org_type2, org_name)  %>% filter(Agency == "FDIC", is.na(org_type2))
 ...
 # caption = "Consumer Financial Protection Bureau (SEC)
...
d %<>% mutate(payday = ifelse(str_detect(comment_url, "SEC-2016-0025|SEC-2019-0006"), "Payday Loan Rules", "All Other Rules"))
...
```{r, FDIC-count-national-association, fig.height=3, fig.width=4, out.width="50%", eval=FALSE}
...
```{r FDIC-count-by-class, fig.height=3,  fig.width=4, out.width="49%", eval=FALSE}
FDIC_resources %>%
...
```{r FDIC-density-by-class, fig.height=3,  fig.width=4, out.width="49%", eval=FALSE}
FDIC_resources %>%
  filter(ASSET > 1000000, BKCLASS %in% c("NM")) %>% 
  densityplot(
...
ind_assoc_resources %>% 
  ...
  densityplot(var = "assets",
              by = 1000,
              title = "Industry Associations (SEC Only)",
               x = "Assets (Thousands)",
...
opensecrets %>% 
  densityplot(var = "MeanContribAmount",
              by = 1000,
              title = "Campaign Donors (SEC Only)",
...
opensecrets %>% 
  densityplot(var = "MeanLobbyingAmount",
              by = 1000,
              title = "Lobbying Spending (SEC Only)",
...
compustat %>% 
  ...
  densityplot(var = "marketcap2",
              by = 1000000, 
              title = "Publicly-traded Companies (SEC Only)",
...
Continued at judgelord.github.io/finreg/SEC/participation-probability.html and judgelord.github.io/finreg/SEC/participation-frequency.html
